...
class InvManagementMasterEnv(gym.Env):
    ...
    '''
    def __init__(self, neptune_instance, max_path_length, inventory, capacity, flag = True, *args, **kwargs):
        '''
        ...
        self.periods = max_path_length
        # self.I0 = [100, 100, 200] #Initial inventory - original value
        self.I0 = inventory
        # self.I0 = [15, 10, 20] #Initial inventory 
        self.p = 2
        self.r = [1.5, 1.0, 0.75, 0.5]
        self.k = [0.10, 0.075, 0.05, 0.025]
        self.h = [0.15, 0.10, 0.05]
        # self.c = [100, 90, 80] #capacity constraint? - original value
        # self.c = [20, 15, 20] #capacity constraint? - original value
        self.c = capacity
        self.flag = flag
        self.L = [3, 5, 10]
        self.backlog = True
    ...
    def _STEP(self,action):
        ...
        self.inventory_constraint = Im1
        self.replenishment_order = R
        self.capacity_constraint = c
        ...
        # print('Im1 ', Im1, 'I ', I, 'II', II, 'full ', self.I)
        # update period
        self.period += 1  
        ...
        #Penalty
        penalty =  np.sum(action[action>=Im1]-Im1[action>=Im1]) + np.sum(action[action>=c]-c[action>=c])
        inventory_next_run = I

        return self.state, reward, done, penalty, {'Node1/Replenishment order': R[0], 'Node2/Replenishment order': R[1], 'Node3/Replenishment order': R[2],
        'Node1/Inventory constraint': Im1[0], 'Node2/Inventory constraint': Im1[1], 'Node1/Capacity constraint': c[0], 'Node2/Capacity constraint': c[1], 'Penalty': penalty, 
        'Node3/Capacity constraint': c[2], 'Node1/Backlogs': B[0], 'Node2/Backlogs': B[1], 'Node3/Backlogs': B[2], 'Node1/LostSales': LS[0], 'Node2/LostSales': LS[1], 'Node3/LostSales': LS[2], 'Node1/Inventory constraint next': inventory_next_run[1], 'Node2/Inventory constraint next': inventory_next_run[2]}, actual_R
