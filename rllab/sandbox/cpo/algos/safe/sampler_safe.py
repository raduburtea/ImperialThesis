...
class BatchSamplerSafe(Sampler):
    def __init__(self, algo, neptune_instance, **kwargs):
        """
        ...
        self.use_safety_baselines = True
    ...
    def record_statistics(self, itr, paths, evs, samples_data): 
        ...
        actions = [path['actions'] for path in self.experience_replay[-1]]
        penalties = [path["env_infos"]['Penalty'] for path in paths]
        node1_ro = [np.mean(path['env_infos']['Node1/Replenishment order']) for path in self.experience_replay[-1]]
        node2_ro = [np.mean(path['env_infos']['Node2/Replenishment order']) for path in self.experience_replay[-1]]
        node3_ro = [np.mean(path['env_infos']['Node3/Replenishment order']) for path in self.experience_replay[-1]]
        node1_ic = [np.mean(path['env_infos']['Node1/Inventory constraint']) for path in self.experience_replay[-1]]
        node2_ic = [np.mean(path['env_infos']['Node2/Inventory constraint']) for path in self.experience_replay[-1]]
        node1_cc = [np.mean(path['env_infos']['Node1/Capacity constraint']) for path in self.experience_replay[-1]]
        node2_cc = [np.mean(path['env_infos']['Node2/Capacity constraint']) for path in self.experience_replay[-1]]
        node3_cc = [np.mean(path['env_infos']['Node3/Capacity constraint']) for path in self.experience_replay[-1]]

        node1_bo = [np.mean(path['env_infos']['Node1/Backlogs']) for path in self.experience_replay[-1]]
        node2_bo = [np.mean(path['env_infos']['Node2/Backlogs']) for path in self.experience_replay[-1]]
        # node3_ro = [np.mean(path['env_infos']['Node3/Replenishment order']) for path in self.experience_replay[-1]]
        # node1_ic = [np.mean(path['env_infos']['Node1/Inventory constraint']) for path in self.experience_replay[-1]]
        # node2_ic = [np.mean(path['env_infos']['Node2/Inventory constraint']) for path in self.experience_replay[-1]]

        node1_ro_max = [np.max(path['env_infos']['Node1/Replenishment order']) for path in self.experience_replay[-1]]
        node2_ro_max = [np.max(path['env_infos']['Node2/Replenishment order']) for path in self.experience_replay[-1]]
        node3_ro_max = [np.max(path['env_infos']['Node3/Replenishment order']) for path in self.experience_replay[-1]]
        ...
        # log everything
        logger.record_tabular('Sample actions ', random.sample(actions, 1))
        ...
        logger.record_tabular('Node1/RO MAX', np.mean(node1_ro_max))
        logger.record_tabular('Node2/RO MAX', np.mean(node2_ro_max))
        logger.record_tabular('Node3/RO MAX', np.mean(node3_ro_max))
        ...
        if self.neptune_instance is not None:
            ...
            self.neptune_instance['Node1/Backlogs'].log(np.mean(node1_bo))
            self.neptune_instance['Node2/Backlogs'].log(np.mean(node2_bo))
