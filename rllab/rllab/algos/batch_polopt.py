...
class BatchPolopt(RLAlgorithm):
    ...
    def __init__(
        ...
        if sampler_args is None:
            sampler_args = dict()
        self.neptune_instance=neptune_instance
        self.sampler = sampler_cls(self, self.neptune_instance, **sampler_args)
    ...
    def train(self):
        ...
        for itr in range(self.current_itr, self.n_itr):
            with logger.prefix('itr #%d | ' % itr):
                ...
                # raise Exception(samples_data)
                # logger.log("DATA samples %s" % str(samples_data[0]))
        ...
        if self.env.name == "NetworkManagement":
