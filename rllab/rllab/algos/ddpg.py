...
class DDPG(RLAlgorithm):
        ...
        for epoch in range(self.n_epochs):
            ...
            for epoch_itr in pyprind.prog_bar(range(self.epoch_length)):
                ...
                if terminal or path_length > self.max_path_length:
                    ...
                    # to the replay pool
                    self.neptune_instance['Node1/Replenishment order'].log(np.sum(node_status['Node1/Replenishment order']))
                    self.neptune_instance['Node2/Replenishment order'].log(np.sum(node_status['Node2/Replenishment order']))
                    self.neptune_instance['Node3/Replenishment order node 3'].log(np.sum(node_status['Node3/Replenishment order']))
                    self.neptune_instance['Node1/Inventory constraint'].log(np.mean(node_status['Node1/Inventory constraint']))
                    self.neptune_instance['Node2/Inventory constraint'].log(np.mean(node_status['Node2/Inventory constraint']))
                    self.neptune_instance['Node1/Capacity constraint'].log(np.mean(node_status['Node1/Capacity constraint']))
                    self.neptune_instance['Node2/Capacity constraint'].log(np.mean(node_status['Node2/Capacity constraint']))
                    self.neptune_instance['Node3/Capacity constraint'].log(np.mean(node_status['Node3/Capacity constraint']))
                    observation = self.env.reset()
                    self.es.reset()
                    sample_policy.reset()
                ...
                next_observation, reward, terminal, penalty, node_status = self.env.step(action)
                # if epoch_itr % 5 == 0:
                #     print(np.maximum(action.astype(np.float64), 0).astype(int), reward)  # qf=qf)
                path_length += 1
                path_return += reward
                ...
                if not terminal and path_length >= self.max_path_length:
                    ...
                    if self.include_horizon_terminal_transitions:
                        pool.add_sample(observation, np.maximum(action.astype(np.float64), 0).astype(int), reward * self.scale_reward, terminal)
                else:
                    pool.add_sample(observation, np.maximum(action.astype(np.float64), 0).astype(int), reward * self.scale_reward, terminal)
    ...
    def evaluate(self, epoch, pool):
        ...
        logger.record_tabular('AverageReturn',
                              np.mean(returns))
        self.neptune_instance["Return"].log(np.mean(returns))
